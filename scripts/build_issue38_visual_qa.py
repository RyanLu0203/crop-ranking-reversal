...
def render_pdf(pdf: Path, temp_dir: Path) -> list[Path]:
    ...
    if executable is None and BUNDLED_PDFTOPPM.exists():
        executable = str(BUNDLED_PDFTOPPM)
    if executable is None:
        raise FileNotFoundError("pdftoppm is required for compiled-page QA")
    subprocess.run(
...
def main() -> None:
    ...
    # Page-detail filenames encode the current page count.  Remove only prior
    # page-detail tiles so a shorter rebuild cannot retain stale pages while
    # preserving the separately generated width and renderer QA artefacts.
    for stale_tile in OUT.glob("*_pages_*.png"):
        stale_tile.unlink()
