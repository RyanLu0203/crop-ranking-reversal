...
multiple optima. KKT validates a point; optimal-face analysis determines whether the
...
first solved point on the declared Gaussian frontier that attains the fixed
15\% variance-reduction target relative to \(x^0\). Let \(x^T\) be the full
loss-CVaR optimum under the declared tail law.
...
x^{MV}&\ne x^T,
\end{align}
where \(x^0\) is the expected-profit policy under the matched Gaussian law.
...
variance reduction, matched Gaussian dependence, evaluation-law loss-CVaR and
allocation disagreement. It does not assert that every diversified portfolio
fails or that tail dependence can be summarized by one coefficient. If
