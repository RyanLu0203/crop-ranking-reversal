...
The theorem is narrow by design. It characterizes acreage directly while
retaining expected-margin ordering, operational bounds and the full joint
loss law. If \(r_\theta(A/2)\) is continuous and strictly increasing along a
named family and crosses \(\kappa\), there is a unique family-specific
threshold \(\theta^*\) solving
\(r_{\theta^*}(A/2)=\kappa\). Without strict monotonicity or crossing, the
scientific object is the reversal set
\(\{\theta:\sup\mathcal K_{\theta,\kappa}<A/2\}\); it may contain several
components. This is why a universal ``higher tail dependence causes
reversal'' statement is not valid.
...
The proof follows convex KKT sufficiency or, in the implemented model, linear
programming strong duality. The theorem matters because incomplete
stationarity can assign a residual margin difference to ``risk'' even when
budget, contract or shared capacity is active. It also clarifies the role of
multiple optima. KKT validates a point; the face audit determines whether the
ranking relation holds throughout all equivalent optima.
...
CVaR is monotone in convex order, yielding the risk and feasible-set
...
family by family, and we retain active-set transitions, multiple optima and
disconnected regions.

\subsection{An executable diversification-failure criterion}

Let \(x^{MV}\) be the policy chosen under a conventional Gaussian
mean--variance assessment and \(x^T\) the full loss-CVaR optimum under the
declared tail law.

\begin{definition}[Diversification failure]
Conventional diversification fails when
\begin{align}
\operatorname{Var}_G[\Pi(x^{MV})]&\le
\operatorname{Var}_G[\Pi(x^0)],\\
\CVaR_{\alpha,T}[L(x^{MV})]&>
\CVaR_{\alpha,T}[L(x^T)],\\
x^{MV}&\ne x^T,
\end{align}
where \(x^0\) is the registered comparison policy. Failure is operationally
strong when \(\CVaR_{\alpha,T}[L(x^{MV})]>\kappa\).
\end{definition}
...
This statement is logically modest but empirically useful. It separates
variance reduction, matched Gaussian dependence, true-law loss-CVaR and
allocation disagreement. It does not assert that every diversified portfolio
fails or that tail dependence can be summarized by one coefficient.

\subsection{Information, complementarity and substitution}

Nature draws state \(\omega\), produces a noisy signal \(S\), and leaves a
fraction of the plan adjustable. The producer observes \(S\), chooses a
signal-contingent action and then realizes margin. Loss-CVaR is evaluated
ex ante over the combined state--signal distribution.

\begin{theorem}[Information value]
If ignoring the signal is admissible, optimized information value is
nonnegative. It is zero when one common action is optimal for every posterior.
If posterior optima are unique and differ on positive-probability signal
events, with strict improvement on at least one event, information value is
strictly positive.
\end{theorem}
...
improvement integrates to positive value. Nested action sets raise both
informed and uninformed optimized values, but their difference need not be
monotone.

\begin{proposition}[Conditional complementarity]
If actions are nested and state--action payoff has increasing differences in
...
These results repair an unconditional supermodularity claim. Complementarity
requires a particular action-set mechanism. Substitution is expected when
flexibility neutralizes precisely the uncertainty that the signal resolves.
