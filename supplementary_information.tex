...
optimized policy. Heuristic benchmark policies are projected onto the feasible
set so that all policies are compared inside the same operational set.
...
Winner take all, projected & 0.300 & 0.100 & 0.600 & 166.6 & -57.3 & yes\\
...
The primary policy table retains a low-penalty mean--variance diagnostic for
comparability. The diversification comparison is separate. Its \(x^0\) is the
matched-Gaussian expected-profit allocation; all
\DiversificationFrontierPoints{} points over \(\gamma\in[0,0.03]\) are solved;
and its selected \(x^{MV}\) is the first point to attain the fixed
...
strong exclusion reversal in \PhaseStrong{}. Complete
family-by-\(\tau\) table, all three ranked-pair face extrema and possible,
universal and selected classifications are retained for every cell.
...
window, scenario count, random seed and solver. With positive crop
lower bounds, none of these cells can satisfy the exclusion
definition.
